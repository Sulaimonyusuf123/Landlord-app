...
import { getUnitById, updateUnit } from "../../../lib/db";
...
const EditUnit = () => {
  ...
  const { control, handleSubmit, setValue, formState: { errors } } = useForm<UnitFormData>();
  ...
  useEffect(() => {
    const loadUnit = async () => {
      try {
        ...
        const unit = await getUnitById(unitId, user.$id);
        if (unit) {
